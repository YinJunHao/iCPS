'''
This script is specific to Protege Ontology Map (Cognitive Engine on Java).
Create/Edit a new script, ontology map and cognitive engine according to each new system.
Ensure that the script matches the ontology map and cognitive engine.
'''
...
class CognitiveEngine():
    ...
    def check_availability(self, job_id, user_id, next_action_seq, next_step_seq):
        ...
        preferred_exec_list = self.get_next_exec(action_details, next_step_seq)
        location_id = self.get_next_location(action_details)
        num_available_exec = self.count_available_exec(preferred_exec_list, location_id)
    ...
    def count_available_exec(self, preferred_exec_list, location_id):
    ...
    def get_next_exec(self, action_details, next_step_seq):
    ...
    def get_next_location(self, action_details):
    ...
    def get_exec_details_list(self, job_id, user_id, action_seq, step_seq):
        ...
        if exec_id_list is not None:
            ...
            for exec_id in exec_id_list:
                ...
                resource_details['class'] = resource_class
    ...
    def get_inference_class(self, job_id, user_id, step_status, action_seq, step_seq, total_action_count, total_step_count):
        ...
        if exec_detail_list is not None:
            for exec_detail in exec_detail_list:
                resource_class_tmp = exec_detail.get('item_type', False)
                if not resource_class_tmp:
                    resource_class_tmp = exec_detail.get('class')
                resource_name_tmp = exec_detail.get('ID')
                resource_name.append(resource_name_tmp)
                resource_class.append(resource_class_tmp.title())
        else:
        ...
        # After the Java does the inference, a list of classes (cognitive engine class) will be returned.
